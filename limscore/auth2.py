...
def login(action):
    with engine.connect() as conn:
        ...
        login_token = None
        insecure_networks = current_app.config.get("LOGIN_TOKEN", "!127.0.0.0/8").split(",")
        for network in insecure_networks:
            if bool(network.startswith("!")) == bool(ip_address(request.remote_addr) in ip_network(network.lstrip("!"))):
                break
        else:
        ...
        if request.method == "POST":
            ...
            if form.validate():
                ...
                elif action == "reset" and current_app.config.get("MAIL_SENDER", False):
    ...
    if current_app.config.get("MAIL_SENDER", False):
...
def set_password(token):
    with engine.begin() as conn:
        ...
        try:
            ...
            return render_page("base.html", title="Invalid or expired link.")
        
        form = ChangePasswordForm(request.form)
        del form["old_password"]
        if request.method == "POST" and form.validate():
            login_token = token_urlsafe(64)
            logic.admin_edit(users,
                             user_id,
                             {},
                             {},
                             conn,
                             calculated_values={"reset_datetime": None,
                                                ...
                                                "password": bcrypt_sha256.hash(form.password1.data),}
                            )
...
def send_setpassword_email(user, conn):
    ...
    token = URLSafeTimedSerializer(config['SECRET_KEY'],
                                   salt="set_password"). \
                dumps([user["email"], reset_datetime])
    ...
    subject = f"{name} Password Link"
    sendmail(user["email"], subject, body)
    sql = users.update().where(users.c.email == user["email"]).values(reset_datetime=reset_datetime)
    conn.execute(sql)
