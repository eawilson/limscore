...
class Fields(MutableMapping):
    """ Ordered collection of form fields that can be accessed by name, either
        as an attribute or a key. Names will be converted to string before
        being saved. None and "" are both illegal and will raise a TypeError.
        May be initialised with a variable number of (name, field) tuples or
        with name=field passed as keyword arguments. Each field added will have
...
class Form(Fields):
    ...
    @property
    def error(self):
        return ", ".join(field.errors for field in self.values() if field.errors)
